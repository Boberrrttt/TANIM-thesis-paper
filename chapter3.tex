...
The Data Flow Diagram visually represents how data flows within the TANIM system. It highlights the interactions between IoT devices, farmers/agricultural authorities, and the system’s core processes, demonstrating how data is retrieved, stored, and analyzed to provide soil health insights and crop/fertilizer recommendations.
	...
	\begin{figure}[H]
		...
		\label{fig:DFD}
		\includegraphics[width=1\textwidth]{figures/DFD-Final.pdf}
	\end{figure}
...
The TANIM predictive framework utilizes a Light Gradient Boosting Machine (LightGBM) architecture, selected for its superior computational efficiency, scalability, and robust performance in multiclass classification environments. To mitigate the inherent class imbalances frequently observed in regional soil datasets, the modeling pipeline integrates a K-Means-based data augmentation strategy. This technique synthesizes representative minority-class samples, yielding a statistically balanced and rigorous training corpus. The algorithmic approach systematically evaluates key soil chemical covariates—specifically Organic Matter (N) fraction, Phosphorus (P) and Potassium (K) concentrations, and pH—to quantify baseline fertility and generate highly accurate crop suitability predictions. By synergizing the predictive capabilities of LightGBM with advanced synthetic data generation, the system outputs precise, site-specific agronomic recommendations designed to optimize crop yield and maximize resource use efficiency.
...
<<<<<<< HEAD
\subsection{Machine Learning Model}
The TANIM system will use a boosting algorithm as the core of its predictive model, chosen for its efficiency, scalability, and high accuracy in handling large and complex datasets. This machine learning approach analyzes multiple factors such as soil type, weather conditions, and historical yield data to support data-driven decision-making. The model first predicts soil health and potential crop performance based on numerical and environmental inputs. The outputs of the model, including soil quality classification and crop suitability predictions, are then used to generate recommendations for appropriate crops and fertilizer applications. These recommendations are further guided by crop nutrient requirements, soil parameter thresholds, and environmental considerations. By leveraging the predictive capability of machine learning with real-time IoT-based data, the system produces accurate, context-aware, and data-driven crop recommendations that help optimize productivity and support sustainable farming practices.
=======
\subsection{Data Acquisition and Storage}
The IoT sensor is tasked with acquiring real-time data by measuring critical indicators of soil health, such as N-P-K levels, pH, temperature, and moisture. Rather than sending data to passive cloud storage, the sensor transmits these readings directly to the machine learning API. The system immediately utilizes this data as input for the trained LightGBM model and rule-based classification services, which analyze the information on the fly to generate precise crop predictions and soil insights. These real-time recommendations are temporarily cached for instant access by the web or mobile client, while also being synchronized to the secure backend database for persistent storage. Ultimately, this seamless integration of hardware, machine learning, and real-time data delivery supports informed decision-making and precise agricultural management.
>>>>>>> 319988d4dfe7fbac054fd7bf5ed11b624160d4f3
