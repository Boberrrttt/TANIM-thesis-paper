...
The system will undergo evaluation using both quantitative and qualitative approaches. The quantitative metrics were selected to ensure a comprehensive assessment of the system’s accuracy, reliability, and consistency. For soil health prediction, accuracy, precision, recall, and F1-score are used to measure how effectively the model classifies soil quality levels, balancing correctness and error sensitivity. For crop yield forecasting, R², RMSE, and MAE evaluate how well the model predicts continuous yield values by analyzing the variance explained and the magnitude of prediction errors. In the case of crop and fertilizer recommendations, consistency ratios and ranking stability indices validate the reliability of the Multi-Criteria Decision Analysis (MCDA) by ensuring stable and logically consistent results under varying conditions. These metrics collectively justify the model’s performance and generalization capability. Cross-validation is also implemented to test robustness across different crop zones.

Meanwhile, qualitative evaluation will involve focus group discussions and structured interviews with farmers and agricultural technicians in Region 10 to assess usability, reliability, and practical value. The integration of both evaluation methods ensures that the system is not only accurate in prediction but also effective and applicable in real-world agricultural practices.
...
Hardware testing and calibration will ensure that the IoT components function accurately and reliably under real farming conditions. The testing will focus on validating the accuracy of the soil sensors, the correctness of the sensor readings, the range and stability of the LoRa transmission, the processing capability and reliability of the ESP32 microcontroller, and the efficiency of the solar-powered battery system.

To conduct this testing, the sensors will be deployed in field testing on actual farms in Region 10. To ensure consistency under varying environmental conditions, readings will be recorded at different times of the day and across several soil types. Cross-checking will be performed using the Department of Agriculture’s soil testing laboratory and standardized soil test kits from the same farms to verify measurement reliability and precision.
